...
function user_jj_lx($var)
{
    ...
    $proall = M('user_jj')->where(array('id' => $var))->find();//加入查询  获取索罗斯支出者打款时间  

    $result = M("userget")->where(array("varid" => $var))->find();//提现查询  获取提现时间
    $ppdd = M("ppdd")->where(array("id" => $proall["r_id"]))->find();//配对信息

    $NowTime = date('Y-m-d', strtotime($proall['date']));//格式化加入时间  确认打开  获取索罗斯支出者排单时间
...
function user_jj_paidui_lx($var,$return=true)
{
    ...
    if(C('pdfhdays') > 0){
       $paidui_fenhong_day = C('pdfhdays');
        ...
        $proall = M('user_jj')->where(array('id' => $var))->find();//加入查询  获取申请索罗斯支出的日期
        $ppdd = M("ppdd")->where(array("id" => $proall["r_id"]))->find();//配对信息 
       ...
       if($paidui_fenhong_day == 1){
            ...
            $paidan_date = date('Y-m-d', strtotime($proall['date']));    //----------------------->申请索罗斯支出的日期
...
function jlj($a, $b, $c)
{
    jlsja($a); //处理索罗斯支出的推荐人是否可以升级为经理的考核
    //索罗斯支出的推荐人资料
...
//第一个参数 索罗斯支出的直接推荐人      推荐奖金额           说明                   第几代          ppdd外键id
...
function jlj3($a, $b, $c, $d, $e)
{
    ...
    $ppddxx = M('ppdd')->where(array('id' => $e))->find();      //获取索罗斯支出者的配对
...
function jlsja($var)
{
    ...
    $zctj = 0;
    $zctjuser = M('user')->where(array('UE_accName' => $var))->select();      //--------------》获取索罗斯支出者的下家
    ...
    $tgbztj = M('ppdd')->where(array('p_user' => $var, 'zt' => '2'))->sum('jb');               //-------------------------->统计索罗斯支出者也就是当前的考核对象的成功交易总额是否大于7000元

    if ($zctj >= C('xiaxian_num') && $tgbztj >= C('my_jb')) {                                            //---------------------->如果下级中交易成功总额大于700的有10以上包括是10个并且当前考核对象索罗斯支出的人
                                                                                     //---------------------->索罗斯支出的总金额大于7000元 就可以升级为经理
...
//jjtuijianratenew  推荐奖 $vart--->索罗斯支出的推荐人
function fftuijianmoney($var,$money,$level){
    $tjratearr = explode(',',C("jjtuijianratenew"));
	$tjdaishuarr = explode(',',C("jjtuijianjbdaishu"));

    $accname_zq=M('user')->where(array('UE_account'=>$var))->find();

	$accountlevelarr = explode(',',C("jjaccountlevel"));
	$levelnum=0;
	foreach($accountlevelarr as $k=>$v){
		if($v==$accname_zq['levelname']){
			$levelnum=$k;
			break;
		}
	}
	$daishu = 0;
	$daishu = $tjdaishuarr[$levelnum];
	if($daishu>=$level){
		$tjmoney = ($money*$tjratearr[$level-1])/100;  //推荐奖金额

		M('user')->where(array('UE_account'=>$var))->setInc('tj_he',$tjmoney); //添加索罗斯支出的推荐人
		$accname_xz=M('user')->where(array('UE_account'=>$var))->find();  //获取推荐人的详细信息

				$note3 = "推荐奖-".$accname_zq['levelname']." 第".$level."代".$tjratearr[$level-1]."%";
				$record3 ["UG_account"] = $var; // 登入转出账户  索罗斯支出的推荐人
    ...
    jsaccountmoney($var,$money,$accname_zq['levelname']);
    if($accname_zq['ue_accname']<>'' && isset($tjratearr[$level])){  //---------->添加了个数组大小的判断
        fftuijianmoney($accname_zq['ue_accname'],$money,$level+1);
    }else{
        return true;
    }
...
// 第一个参数 索罗斯支出人 第二参数是帮助金额              ----------------------》经分析此函数多次计算直接推荐人的经理代数奖
function lkdsjfsdfj($p_user, $jb)
{

    $ppddxx['p_user'] = $p_user; //索罗斯支出人
    $ppddxx['jb'] = $jb; //第二参数是帮助金额
    //经理奖金订单
    $tgbz_user_xx = M('user')->where(array('UE_account' => $ppddxx['p_user']))->find();//索罗斯支出人详细
