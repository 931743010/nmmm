...
class CommonController extends Controller {
	...
	function getConfig($name){
		$data = M("Config")->where("name = '%s'",$name)->find();
		return $data ? $data['value'] : '';
	}
	function updateConfig($name,$value){
		$save['value'] = $value;
		return M("Config")->where("name = '%s'",$name)->save($save);
	}
...
// 	}
		...
		$this->assign("scheduleStatus",$status);
...
// 		$time2 = date('H');
		...
		$this->code_zs = M('code')->where(array('onwer' => $_SESSION['uname'], 'status' => 1))->count();
		$this->code1=M('code')->where(array('onwer'=>$_SESSION['uname'],'status'=>'1'))->find();
